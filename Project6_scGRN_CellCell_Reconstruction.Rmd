...
check <- merge(syn_resultlink,gold) # if you want to see the TP for GENIE (syn_resultlink->linka) and for ARACNE (syn_resultlink->linkb) 
...
subnet_threshold = 0.50
close_pairs = bertie_resultlink2[which(bertie_resultlink2[,3]>=subnet_threshold),]
subnet1 = c("Cpox", "Tmem14c", "Smim1", "Tsen34", "Cited4", "Sod2", "Hbb-bh1", "Hba-x", "Hba-a1", "Hba-a2", "Hmbs", "Blvrb")
subnet2 = c("Ctsz", "Amn", "Dab2", "Glrx", "Apom", "Apoa1", "Rbp4", "Ttr", "Ctsh", "Spink1")
subnet3 = c("mt-Atp6", "mt-Cytb", "mt-Nd4")
subnet4 = c("Pa2g4", "Ncl", "Nolc1", "Nop56", "Hspd1", "Umps")
...
for (i in 1:length(clusters)){
  ...
  cluster_exp = as.matrix(bertie_expMat[,index])
  expMerged = cbind(expMerged, cluster_exp)
}
colnames(expMerged) = clusters
rownames(expMerged) = rownames(bertie_expMat)
dbs<-c("Mouse_Gene_Atlas")

# this below gene enrichment analysis prints out like 50 different ones lol because I'm comparing each of the 10 clusters to each other.... I don't think that's how we're supposed to do it but there's no way to do it on the subnetworks because they don't have the same dimensions
for (i in 1:length(clusters)){
  for (j in 1:length(clusters)){
    if (i != j){
      ...
      print(cname1)
      print(cname2)
      upIn12 = findDiffGenes(expMerged,cname1 = cname1, cname2 = cname2)
      enr12<-enrichr(upIn12,dbs[1])
      print(enr12[[1]][1:15,1:4])     
    }
  }
}
```
