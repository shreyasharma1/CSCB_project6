...
findPR <- function(threshold, network, gold){
  ...
  for(i in 1:nrow(nonzero)){
    for(j in 1:nrow(GRN)){
      if(as.character(nonzero$TF[i]) == as.character(GRN$TF[j]) && as.character(nonzero$TG[i]) == as.character(GRN$Target[j])){# if true positive
        TP <- TP + 1
      }
      else if(as.character(nonzero$TF[i]) == as.character(GRN$TF[j]) && as.character(nonzero$TG[i]) != as.character(GRN$Target[j])){# if makes a pair where one gene is in gold standard and one isnt, FP
        FP <- FP + 1
      }
      else if(as.character(nonzero$TF[i]) != as.character(GRN$TF[j]) && as.character(nonzero$TG[i]) == as.character(GRN$Target[j])){# if makes a pair where one gene is in gold standard and one isnt, FP
        FP <- FP + 1
...
smallNet <- function(df){
  df100Rows <- df[sample(1:nrow(df), 100), ]
  dfFinal <- df100Rows[ ,sample(1:ncol(df), 100)]
  return(dfFinal)
}
...
syn_aracneMat <- aracne(syn_mim_part2, eps=0.1)
...
# intersect two links
syn_resultlink <- merge(syn_linka,syn_linkb)
```
...
bertie_expMat1 <- t(bertie_expMat)

# performing CLR on bertie data
bertie_mim <- build.mim(bertie_expMat1,estimator="spearman")
...
regulators <- rownames(bertie_expMat)
bertie_expMat = data.matrix(bertie_expMat, rownames.force = NA)
bertie_weightMat <- GENIE3(bertie_expMat, regulators=regulators, nCores=4, verbose=TRUE)

# normalize the value of threshold
Gen_th <- (((threshold - 1) * (max(bertie_weightMat)-min(bertie_weightMat))) / 9) + min(bertie_weightMat)

#get link above the threshold
bertie_linkList <- getLinkList(bertie_weightMat)
...
# Reconstruct grn using aracne
bertie_aracneMat <- aracne(bertie_mim, eps=0.1)
# normalize the value of threshold
...
library(RaceID)
seurat_bertie <- as.Seurat(sce_bertie, counts = "counts", data = "logcounts")
bertie_expMat = as.matrix(seurat_bertie[["RNA"]]@data)
library("Matrix")
expMat = as(bertie_expMat, "dgCMatrix")
sc <- SCseq(expMat)
